...
## Current status (v0.3.36) — peer SCO **OPENED** (0.3.35); auto-close proof; **extra baseline v0.3.27**
...
| **SCO probe** | **OPENED works** (`slave_open=1`); auto-close after proof (do not leave up with extra) |
...
**v0.3.27** remains the freeze point for **extra L2CAP** CROS audio. **v0.3.35**
proved peer SCO **OPENED** (`slave_open=1`); leaving it up with extra wedged the
buds — **v0.3.36** auto-closes after proof. Latency next: CROS **on** SCO, not
both. **v1.0 note:** configurable poor side must keep the IBRT-master×TX guard —
see [architecture-cros.md](docs/architecture-cros.md#hard-constraint-v0333--must-keep-for-v10).
...
Latest zip: [`flash-packages/pinebuds-cros-v0.3.36.zip`](flash-packages/pinebuds-cros-v0.3.36.zip) · [CHANGELOG](CHANGELOG.md) · [VERSION](VERSION)
...
See [latency-and-next.md](docs/latency-and-next.md). Flash **v0.3.36** for safe SCO
OPENED proof (auto-close); **v0.3.27** for daily extra-only. Constraint for v1.0
...
1. Download **[pinebuds-cros-v0.3.36.zip](flash-packages/pinebuds-cros-v0.3.36.zip)** (includes `bestool.exe`).
